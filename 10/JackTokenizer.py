# =============================================================================
# Description : produces a XML description of a Jack source file. Example 
# from the project 10 documentation :
# 
# Jack : let quit = "yes";
# 
# XML :
...
# 	<symbol> ; </symbol>
# 
# Implementation based on a personal C source file (scanner.c) which is part of 
# a script interpreter project and which works as follows :
# 
# The main() function creates the list of tokens from the input file by calling 
# the scan() function from scanner.c, passing it a preprocessed buffer 
# containing the entire script (including line breaks and comments).
# 
# 	main() =>
# 	    (...)
# 	    tokens = scan(preprocBuffer, "");
# 	    (...)
# 
# 
# scanner.c / scan() => This function primarily contains a do... while() loop 
# described below, preceded by a call to :
# 
# 	getNoWhiteChar() =>
# 		readChar();
# 		skipWhite();
# 
# 
# - The readChar() function feeds Look (the current character being 
# processed) from lookPtr, which points to the next character in the input 
# within scanBuffer[] (the input buffer).
#   
# 	readChar() =>
# 		Look = NULLCHAR;
# 		if (lookPtr < strlen(scanBuffer)) Look = scanBuffer[lookPtr++];
#   
# 
# - skipWhite() calls readChar() in a loop as long as Look == ' '.
# 
# 
# The do... while() loop mentioned above has the following structure:
# 
# 	do {
# 		next();
# 		switch (TokenType) {
# 		case FOR: nFor++; break;
# 		case NEXT: nFor--; break;
# 		(...)
# 		addToken(&tok_list, idSrc);
# 	} while (TokenType != PROGEND);
# 
# 
# One can see that the next() function builds the token currently being 
# processed. Certain counters are updated based on its type, and then it is 
# added to the tok_list list returned by the scan() function. The loop 
# continues until a specific token indicating the end of the input script is 
# reached.
# 
# The core of the processing is the next() function, which generally works by 
# testing the type of character contained in Look. The character type 
# determines whether we are dealing with an identifier, a numeric literal, a 
# string, etc., and then calls a specialized function (e.g., scanId() for an 
# identifier). Such a function adds Look to the current token 
# [TokenValuePush(Look)] and then recursively calls readChar() in a loop as 
# long as Look matches the detected token type.
# 
# =============================================================================
...
# Returns True if the input string contains an integer between 0 and 32767 :
...
def readJackFile(strFilePath):
	...
	try:
		...
		errMsg(myself, "Failed to open or read " + strFilePath + " !")
	
	return strBuffer

# Reads the next character in the input buffer or NULLCHAR at the end of the 
# buffer.
def readChar():
	myself = "readChar()"
	...
	if g_nBufferPtr < len(g_jackBuffer):
		...
		g_nBufferPtr += 1
		# Pour le cas où g_strLook contient '\n' :
		strDisp = g_strLook
		if g_strLook == "\n":
			strDisp = "<LF>"
...
# Tells if the current token is a symbol, a keyword of none of both depending 
# on its value :
...
def skipComment():
	...
	if g_strLook == "/":
		...
		# NB: In C, "nested comments" are not allowed. We assume the same for Jack...
...
# __ Adds the current character to the value of g_Token. Uses variable arguments
# to allow passing a value other than g_Token.tokenValue, see scanSlash() for example:
# def tokenValuePush():
...
def nextToken():
	...
	if g_strLook.isalpha() or g_strLook == "_":
		...
		# checking if g_strLook is in symbolSet, otherwise the analyzer
...
# to the output file. Returns the number of errors found.
#
...
# def scanJack(jackFilePath, xmlFile):
def scan():
	...
	nextToken()
	# Note: The while loop logic is slightly different from scan()!
...
def saveTokenList(xmlFile):
	...
	traceMsg(myself, "opening [" + xmlFile + "] for writing...")
	try:
		fXML = open(xmlFile, "w")
	...
	for token in g_tokenList:
		...
		# fXML.write("\t<" + token2XMLTag(token.tokenType) + ">" + token.tokenValue + "</" + token2XMLTag(token.tokenType) + ">\n")
...
for jackFile in jackList:
	...
	try:
		...
		traceMsg(myself, "Saving token list to " + xmlFile)
